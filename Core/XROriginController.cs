    ...
    public class XROriginController : MonoBehaviour
    {
        ...
        void MoveOffsetHeight(float y)
        {
            if (m_CameraFloorOffsetObject != null)
            {
                ...
                desiredPosition.y = (float) y +1.7f;
        ...
        void OnBeforeRender()
        {
            ...
            if (TrackablesParent.hasChanged)
            {
                TrackablesParentTransformChanged?.Invoke(
                    new ARTrackablesParentTransformChangedEventArgs( ((XROrigin) this), TrackablesParent));
                TrackablesParent.hasChanged = false;
            }
        }

        public static explicit operator XROrigin(XROriginController v)
        {
